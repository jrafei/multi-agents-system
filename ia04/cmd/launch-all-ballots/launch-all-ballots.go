...
func main() {
	...
	list_ballot := [10]string{"majority", "borda", "approval", "condorcet", "copeland", "stv", "kramer-simpson",
		"young-condorcet", "dodgson", "kemeny-young"}
	//Créer un agent qui demande la création de plusieurs ballots
	ballot_agent := agent.NewAgent("ballot_agt1", "ballot_agt", nil, nil)

	list_rules := [10]string{"majority", "borda", "approval", "condorcet", "copeland", "stv", "kramer-simpson",
		"young-condorcet", "dodgson", "kemeny-young"}
	...
	for i, _ := range votersAgts {
		func() {
			...
			go votersAgts[i].Vote("scrutin7", url_server)
			go votersAgts[i].Vote("scrutin8", url_server)
			go votersAgts[i].Vote("scrutin9", url_server)
			go votersAgts[i].Vote("scrutin10", url_server)
		}()
	}
